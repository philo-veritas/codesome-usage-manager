...
func TestAPIKeyRepositoryListActiveSwitchTargets(t *testing.T) {
	...
	if _, err := repo.Create(ctx, CreateAPIKeyParams{
		...
		t.Fatalf("create inactive api key: %v", err)
	}
	if _, err := repo.Create(ctx, CreateAPIKeyParams{
		UserID:        inactiveUser.ID,
		CodesomeKeyID: 6734,
		...
		t.Fatalf("create inactive user api key: %v", err)
	}

	targets, err := repo.ListActiveSwitchTargets(ctx)
	if err != nil {
		t.Fatalf("list switch targets: %v", err)
	}
	if len(targets) != 1 || targets[0].CodesomeKeyID != 6732 || targets[0].Name != "Alice" {
		t.Fatalf("unexpected switch targets: %+v", targets)
	}
}
